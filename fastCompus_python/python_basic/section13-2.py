...
# DB생성 & Autocommit
# 본인 DB 파일 경로
conn = sqlite3.connect('F:/VSCode_EH/fastCompus_python/python_basic/resource/records.db', isolation_level=None)
...
with open('./fastCompus_python/python_basic/resource/word.txt', 'r') as f:  # 문제 txt 파일 로드
    for c in f:
...
while n <= 5:                                # 5회 반복
    ...
    if str(q).strip() == str(x).strip():     # 입력 확인(공백제거)
        winsound.PlaySound(                  # 정답 소리 재생
            './fastCompus_python/python_basic/sound/good.wav',
        ...
        winsound.PlaySound(                  # 오답 소리 재생
            './fastCompus_python/python_basic/sound/bad.wav',
            winsound.SND_FILENAME
        )
